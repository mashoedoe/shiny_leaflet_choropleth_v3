...
# Add CSS to give floating Panels a style
    ...
    leafletOutput(outputId = 'map1', width = "100%", height = "725px"),
    absolutePanel(
        ...
        output$map1 <-renderLeaflet({
            leaflet() %>%
                ...
                addTopoJSON(group = 'town_tj',
                            topojson=town_tj,stroke=T,weight=1,color="white",
...
#        label="event22"
        ...
        proxy %>%
            ...
            addPolygons(group = 'single',
                        stroke=T,weight=3,color="yellow",opacity=1,
    ...
    observeEvent(input$map1_shape_mouseout,label="event26", {
        proxy <- leafletProxy(mapId = 'map1')
        proxy %>%
            clearGroup('single')
    })
